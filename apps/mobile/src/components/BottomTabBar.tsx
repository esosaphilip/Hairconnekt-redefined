...
export function BottomTabBar({ activeTab, unreadMessages = 0 }: Props) {
  ...
  const tabs = [
    { key: 'index', title: 'Startseite', icon: 'home', route: '/(client)/' },
    { key: 'search', title: 'Suchen', icon: 'search', route: '/(client)/search' },
    { key: 'appointments', title: 'Termine', icon: 'calendar', route: '/(client)/appointments' },
    { key: 'chat', title: 'Nachrichten', icon: 'message-circle', route: '/(client)/chat' },
    { key: 'profile', title: 'Profil', icon: 'user', route: '/(client)/profile' },
  ];

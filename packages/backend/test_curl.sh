...
if [ -z "$TOKEN" ]; then
    ...
    LOGIN_RES=$(curl -s -X POST http://localhost:3000/api/v1/auth/login \
      -H "Content-Type: application/json" \
      -d '{"email":"neu@test.de","password":"Test1234!"}')
    TOKEN=$(echo $LOGIN_RES | grep -o '"accessToken":"[^"]*' | cut -d'"' -f4)
fi

echo -e "\nTEST 2:"
RES2=$(curl -s -X POST http://localhost:3000/api/v1/providers/register \
...
echo -e "\nTEST 4:"
RES4=$(curl -s http://localhost:3000/api/v1/providers/me \
  -H "Authorization: Bearer $TOKEN")
echo "$RES4"
